...
def examine_pop(pop, fig_prop=None, fig_pflx=None, fig_samp=None, label=None, **kwargs):
    df = pop.properties
    ...
    for ax, pflx in zip(axes_pflx.flatten(), pflx_name):
        ...
        ax.set_xlabel(f"log({pflx})".replace('_',' '))
        ax.set_xlim(np.log10(df[pflx].min()), np.log10(df[pflx].max()))
        ax.set_yscale('log')
        ax.legend()

#     samp_name = set([key[5:] for key in df.columns if ('pdet_' in key)])
#     print('Sampe names {}'.format(samp_name))
#     if fig_samp is None:
...
#         if 'ECLAIRs' in samp:
#             instrum = '_'.join(['pht', 'cts', samples[samp.split('_')[0]]['instrument']])
#             weight = 'pdet_'+samp+'_tot'
#             ax.set_xlabel(f"log(cts {samp})".replace('_',' '))
#         else:
#             pflx_min = np.log10(float(samples[samp]['pflx_min']))
#             instrum = '_'.join(['pht','pflx', samples[samp]['instrument']])
#             weight = 'pdet_'+samp
#             ax.set_xlabel(f"log(pflx {samp})".replace('_',' '))
#         ax.hist(np.log10(df[instrum]), weights=df[weight], bins=np.arange(pflx_min, 7, 0.15), label=label, density=True, **kwargs)
