PROGRAM Core
  ...
  CHARACTER(Len=255)       :: Path_data = '../data/'                           ! Path for input data
  ...
  ! --- SVOM/ECLAIRs --- !
  CHARACTER(len=150)                   :: Path_ECLAIRs = '../data/ECLAIRs/'
...
CONTAINS  
  ...
  SUBROUTINE MonteCarlo(hist_flag)
    ...
    CHARACTER(20)                                    :: GRB_PropFile = "GRB_Properties"        ! Filename for saving the properties
    ...
    IF(run_mode == one_run) THEN
       IF(Save_all_GRB .EQV. .TRUE.) THEN
          WRITE(GRB_PropFile(15:16),'(I2)') rank
          OPEN(UNIT=66, FILE=TRIM(path)//TRIM(GRB_PropFile)//".dat", FORM='unformatted')                ! Save all GRB properties, be careful the file is heavy (several GB)
       END IF
...
!!$          CLOSE(55)
 ...
 SUBROUTINE Prepare_Spec()
   ...
   OPEN(UNIT=55, FILE=TRIM(Path_data) // '/alpha_GBM.txt')
   READ(55,*) skip ! skip line
   READ(55,*) skip ! skip line
   READ(55,*) skip ! skip line
   DO i=0, N_GBM_alpha
   ...
   OPEN(UNIT=55, FILE=TRIM(Path_data) // '/beta_GBM.txt')
   READ(55,*) skip ! skip line
   READ(55,*) skip ! skip line
   READ(55,*) skip ! skip line
   DO i=0, N_GBM_beta
 ...
 SUBROUTINE Normalize_Model()
   ...
   IF(Sample_Included(Sample_Stern)) THEN
     ...
     ! IF(run_mode == one_run)  WRITE(*,'(A,ES12.5,A)') "[                 From BATSE23 (Stern data)   :          eta_0 =", k_Stern*Nb_GRB/pseudo_collapse_rate, "                        ]"
      IF(run_mode == one_run)  WRITE(*,'(A,ES12.5,A)') "[                 From BATSE23 (Stern data)   :   R_GRB/R_coll =", k_stern*Nb_GRB/collapse_rate_from_SFR , "                        ]"
...
SUBROUTINE InitECLAIRs(nsigmasECLAIRs)
  ...
  OPEN(UNIT=100,FILE=TRIM(Path_ECLAIRs) // "ECLAIRs_rf_eff.txt")
  NeffECLAIRS = 0
  DO
...
999  CLOSE(100)
  ...
  OPEN(UNIT=100,FILE=TRIM(Path_ECLAIRs) // "ECLAIRs_rf_bkg.txt")
  NbkgECLAIRS = 0
  DO
...
888  CLOSE(100)
  ...
  OPEN(UNIT=100,FILE=TRIM(Path_ECLAIRs) // "ECLAIRs_rf_offaxis.txt")
  DO i=-NoffECLAIRs,NoffECLAIRs
...
!    WRITE(*,*) i,"OK"
  ...
  OPEN(UNIT=100,FILE=TRIM(Path_ECLAIRs) // "ECLAIRs_rf_omega.txt")
  DO i=-NoffECLAIRs,NoffECLAIRs
