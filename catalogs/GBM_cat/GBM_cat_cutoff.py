...
#################### Stern #######################
filename_S = '/nethome/palmerio/1ere_annee/Frederic/catalogs/BATSE_cat/Stern_cat.txt'
...
#################### GBM #######################
filename  = '/nethome/palmerio/1ere_annee/Frederic/catalogs/GBM_cat/GBM_cat.txt'
...
#################### BATSE ####################
filename2 = '/nethome/palmerio/1ere_annee/Frederic/catalogs/BATSE_cat/BATSE_cat.txt'
...
filename3 = '/nethome/palmerio/1ere_annee/Frederic/catalogs/Swift_cat/Swift_pflx_cat.txt'
...
file_Stern  = '/nethome/palmerio/1ere_annee/Frederic/GRB_population_code/observational_constraints/lognlogp.stern.dat'
file_Komm   = '/nethome/palmerio/1ere_annee/Frederic/GRB_population_code/observational_constraints/Kommers2.dat'
...
def chi2_parameter_adjust_logNlogP(hist, hist_err, name, min_pflx=1, max_pflx=30, min_param=0.0001, max_param=10, nb_pts=500, show_plot=False):
	"""
	...
	if show_plot:	
		...
		ax34.scatter(parametre, chi2, marker='o', label='best_param = %.3lf\nfor chi2 = %.1lf\nwith %d dof' %(best_parametre,min_chi2, n_bins_used-1))
...
def plot_logN_logP():
	...
	line1, unused_1, unused_2 = ax2.errorbar(bins_middle, hist_GBM,       xerr=[bins_errm, bins_errp],           yerr=hist_GBM_err,   marker=None, capsize=3, alpha=0.9, zorder=1, linewidth=0.6, fmt='.')
	#line2, unused_1, unused_2 = ax2.errorbar(bins_middle, hist_BATSE,     xerr=[bins_errm, bins_errp],           yerr=hist_BATSE_err, marker=None, capsize=1, alpha=0.9, zorder=1, linewidth=0.6, fmt='.', label=r'BATSE $<\Omega>*\mathrm{T_{utile}} =$%.1e [sr yr]'%(0.67*4*np.pi*6.54))
	line3, unused_1, unused_2 = ax2.errorbar(bins_middle, hist_Stern,     xerr=[bins_errm, bins_errp],           yerr=hist_Stern_err, marker=None, capsize=3, alpha=0.9, zorder=1, linewidth=0.6, fmt='.')
	line4, unused_1, unused_2 = ax2.errorbar(bins_middle_Komm, hist_Komm, xerr=[bins_errm_Komm, bins_errp_Komm], yerr=hist_Komm_err,  marker=None, capsize=3, alpha=0.9, zorder=1, linewidth=0.6, fmt='.')
	line5, unused_1, unused_2 = ax2.errorbar(bins_middle, hist_S,         xerr=[bins_errm, bins_errp],           yerr=hist_S_err,     marker=None, capsize=3, alpha=0.9, zorder=1, linewidth=0.6, fmt='.')
	#line6, unused_1, unused_2 = ax2.errorbar(bins_middle, hist_Sw,        xerr=[bins_errm, bins_errp],           yerr=hist_Sw_err,    marker=None, capsize=1, alpha=0.9, zorder=1, linewidth=0.6, fmt='.', label=label_Swift) # temps utile BATSE * ajustement
	
	ax2.scatter(bins_middle, hist_GBM,      color=line1.get_color(), zorder=5, marker='s', s=40,label=label_GBM)
	#ax2.scatter(bins_middle, hist_BATSE,    color=line2.get_color(), zorder=5, marker='s', s=40)
	ax2.scatter(bins_middle, hist_Stern,    color=line3.get_color(), zorder=5, marker='d', s=40,label='Stern+01 (value from fig. 23 in paper)')
	ax2.scatter(bins_middle_Komm, hist_Komm,color=line4.get_color(), zorder=5, marker='o', s=40,label=r'Kommers+00 (value from tab. 2 in paper multiplied by $4\pi$)')
	ax2.scatter(bins_middle, hist_S,        color=line5.get_color(), zorder=5, marker='*', s=80,label=r'Stern catalog corrected for useful time (9.1 yr) and  $\Omega=0.67*4\pi$')
	...
	#ax2.axvspan(pflx_min_GBM_adjust, pflx_max_GBM_adjust, color='gray', alpha=0.2, zorder=0, label='Range for adjustment of GBM catalog')
	
	
	ax2.set_yscale('log')
	ax2.set_xscale('log')
	leg = ax2.legend(loc='lower left', numpoints=1)
	...
	ax2.set_ylabel(r'$\Delta N/\Delta \,(\mathrm{log\,}P) \mathrm{[GRB/yr]}$'+ ' in $4\pi$', **font)#\,\mathrm{[yr^{-1}]}
...
def Swift_compare():
	...
	file_redshift ='/nethome/palmerio/1ere_annee/Frederic/catalogs/Swift_cat/Swift_redshift_cleaned_table.txt'
